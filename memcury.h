...
namespace Memcury
{
    ...
    public:
        ...
        static auto FindPointerRef(void* Pointer, int useRefNum = 0, bool bUseFirstResult = false, bool bWarnIfNotFound = true) -> Scanner // credit me and ender
        {
            ...
            if (bWarnIfNotFound)
            {
                if (add == 0)
                {
                    log_warn("FindPointerRef return nullptr\n");
            ...
            if (bWarnIfNotFound)
            {
                if (add == 0)
                {
                    if constexpr (bIsWide)
                    {
                        ...
                        log_warn("Failed to find String %s\n", str.c_str());
        ...
        auto RelativeOffset(uint32_t offset) -> Scanner
        {
            if (!_address.Get())
            {
                log_warn("Trying to relative offset an invalid offset!\n");
                return *this;
            }
    ...
    class TrampolineHook
    {
        ...
        void* AllocatePageNearAddress(void* targetAddr)
        {
            ...
            uint64_t minAddr = (uint64_t)fmin(startAddr - 0x7FFFFF00, (uint64_t)sysInfo.lpMinimumApplicationAddress);
            uint64_t maxAddr = (uint64_t)fmax(startAddr + 0x7FFFFF00, (uint64_t)sysInfo.lpMaximumApplicationAddress);
