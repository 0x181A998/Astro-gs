...
static void ServerSetInAircraft(AFortPlayerStateAthena* PlayerState, bool aircragft)
{
	...
	/*for (int i = 0; i < bots; i++) {
		...
		customization->SkillLevel = 0.5;

		FFortAthenaAIBotRunTimeCustomizationData runtimeData;
		runtimeData.CustomSquadId = i + PlayersOnBus + 1;
		SpawnBot(GetGameMode()->ServerBotManager, PC->K2_GetActorLocation(), PC->K2_GetActorRotation(), customization, runtimeData);
	}*/
	...
	if (Aircraft->DefaultBusSkin != OverrideBattleBusSkin) {
		Aircraft->DefaultBusSkin = OverrideBattleBusSkin;
		//Aircraft->SpawnedCosmeticActor->ActiveSkin = OverrideBattleBusSkin;
	}*/
	...
	{
		PC->ClientReturnToMainMenu(TEXT("Invalid pickaxe, please report this to a dev"));
		return;
	}
	if (!bTravis) GiveItem(PC, PC->CosmeticLoadoutPC.Pickaxe->WeaponDefinition);
